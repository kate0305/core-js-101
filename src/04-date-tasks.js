...
function timeSpanToString(startDate, endDate) {
  ...
  let mlseconds = time.getUTCMilliseconds();
  if (mlseconds < 10) { mlseconds = `00${mlseconds}`; }
  if (mlseconds > 10 && mlseconds < 100) { mlseconds = `0${mlseconds}`; }
  return `${time.toLocaleTimeString([], { timeZone: 'UTC' })}.${mlseconds}`;
}
